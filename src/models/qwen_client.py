...
class QwenTextClient(ModelClient):
    ...
    def __init__(
        ...
        *,
        ...
        timeout: int = 120,  # 增加到120秒，大模型需要更长时间
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    # 最后一次重试失败，抛出异常
                    raise RuntimeError(
                        f"Qwen API request failed after {self.max_retries} attempts: {e}"
                    ) from e
        
        # 如果所有重试都失败
        if last_exception:
            raise RuntimeError(
                f"Qwen API request failed after {self.max_retries} attempts: {last_exception}"
            ) from last_exception
...
class QwenVisionClient(ModelClient):
    ...
    def _generate_via_dashscope(self, prompt: str, size: str) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # 下载图片时也添加重试机制
                for download_attempt in range(self.max_retries):
                    try:
                        image_bytes = requests.get(image_url, timeout=self.timeout).content
                        return self._save_image(image_bytes, prompt, size)
                    except (Timeout, SSLError, RequestException) as e:
                        if download_attempt < self.max_retries - 1:
                            time.sleep(self.retry_delay * (2 ** download_attempt))
                            continue
                        else:
                            raise RuntimeError(
                                f"Failed to download image after {self.max_retries} attempts: {e}"
                            ) from e
                ...
                if attempt < self.max_retries - 1:
                    ...
                    raise RuntimeError(
                        f"Qwen image generation failed after {self.max_retries} attempts: {e}"
                    ) from e
        
        if last_exception:
            raise RuntimeError(
                f"Qwen image generation failed after {self.max_retries} attempts: {last_exception}"
            ) from last_exception
        raise RuntimeError("Unexpected error in _generate_via_dashscope")
    ...
    def _generate_via_compatible(self, prompt: str, size: str) -> Dict[str, Any]:
        ...
        raise RuntimeError("Unexpected error in _generate_via_compatible")
